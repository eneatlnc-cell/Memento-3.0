...
class NodeForegroundService : Service() {
  ...
  override fun onCreate() {
    super.onCreate()
    ensureChannel()
    val initial = buildNotification(title = "灵机 Agent", text = "Starting…")
    ...
    notificationJob =
      scope.launch {
        combine(
          ...
          val title =
            when {
              state.connected && state.mode == VoiceCaptureMode.TalkMode -> "灵机 Node · Talk"
              state.connected -> "灵机 Node · Connected"
              else -> "灵机 Node"
            }
          val text =
  ...
  companion object {
    ...
    private const val ACTION_STOP = "com.myagent.app.action.STOP"
    private const val ACTION_SET_VOICE_CAPTURE_MODE = "com.myagent.app.action.SET_VOICE_CAPTURE_MODE"
    private const val EXTRA_VOICE_CAPTURE_MODE = "com.myagent.app.extra.VOICE_CAPTURE_MODE"
