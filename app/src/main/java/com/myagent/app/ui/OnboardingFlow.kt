...
import com.myagent.app.GatewayConnectionProblem
import com.myagent.app.GatewayNodeApprovalState
import com.myagent.app.LocationMode
import com.myagent.app.MainViewModel
import com.myagent.app.R
import com.myagent.app.SensitiveFeatureConfig
import com.myagent.app.node.DeviceNotificationListenerService
import com.myagent.app.ui.design.ClawDesignTheme
import com.myagent.app.ui.design.ClawErrorState
import com.myagent.app.ui.design.ClawListItem
...
fun OnboardingFlow(
  ...
  ClawDesignTheme(dark = onboardingDark) {
    ...
    var loginNickname by rememberSaveable { mutableStateOf("") }
    ...
    when (step) {
      OnboardingStep.Welcome ->
        ClawDesignTheme(dark = true) {
          WelcomeScreen(
            modifier = modifier,
            onConnect = { step = OnboardingStep.MyAgentLogin },
          )
        }
      OnboardingStep.MyAgentLogin ->
        ClawDesignTheme(dark = true) {
          MyAgentLoginScreen(
            modifier = modifier,
            phone = loginPhone,
            ...
            onPhoneChange = { loginPhone = it; loginError = null },
            onPasswordChange = { loginPassword = it; loginError = null },
            onNicknameChange = { loginNickname = it },
            onToggleRegister = { isRegistering = !isRegistering; loginError = null },
            onLogin = {
              loginLoading = true
              loginError = null
              viewModel.viewModelScope.launch {
                val result = if (isRegistering) {
                  com.myagent.app.api.MyAgentAuthManager.register(
                    phone = loginPhone,
                    password = loginPassword,
                    nickname = loginNickname.ifBlank { "灵机用户" },
                  )
                } else {
                  com.myagent.app.api.MyAgentAuthManager.login(loginPhone, loginPassword)
                }
                result.fold(
                  onSuccess = {
                    loginLoading = false
...
private fun WelcomeLogo() {
  Surface(
    ...
    Box(modifier = Modifier.fillMaxSize().padding(12.dp), contentAlignment = Alignment.Center) {
      Image(painter = painterResource(id = R.drawable.myagent_logo), contentDescription = "灵机 Logo", modifier = Modifier.fillMaxSize())
    }
  }
}
...
private fun copyApprovalCommand(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("灵机 pairing approval command", command))
...
private fun copyGatewayDiagnostic(
  ...
  val clipboard = context.getSystemService(Context.CLIPBOARD_SERVICE) as ClipboardManager
  clipboard.setPrimaryClip(ClipData.newPlainText("灵机 gateway diagnostic", diagnostic))
...
// ── MyAgent Login Screen ──────────────────────────────────────────────

@Composable
private fun MyAgentLoginScreen(
  modifier: Modifier = Modifier,
  phone: String,
  password: String,
  nickname: String,
  ...
  Column(
    ...
    Icon(
      painter = painterResource(id = R.drawable.myagent_logo),
      contentDescription = "灵机 Logo",
      modifier = Modifier.size(64.dp),
      tint = Color.Unspecified,
    )
    Spacer(modifier = Modifier.height(20.dp))
    Text(
      text = if (isRegistering) "注册灵机账号" else "登录灵机账号",
      style = ClawTheme.type.title.copy(fontSize = 22.sp),
      color = ClawTheme.colors.text,
    )
    Spacer(modifier = Modifier.height(6.dp))
    Text(
      text = "登录后可使用技能市场、灵豆钱包等功能",
      style = ClawTheme.type.body,
      color = ClawTheme.colors.textMuted,
    )
    ...
    // Nickname (only when registering)
    if (isRegistering) {
      ClawTextField(
        value = nickname,
    ...
    ClawPrimaryButton(
      text = if (isLoading) "请稍候…" else if (isRegistering) "注册" else "登录",
      onClick = onLogin,
    ...
    Text(
      text = if (isRegistering) "已有账号？登录" else "没有账号？注册",
      style = ClawTheme.type.caption,
      color = ClawTheme.colors.primary,
      modifier = Modifier
        .clickable(enabled = !isLoading) { onToggleRegister() }
        .padding(8.dp),
    )
    Spacer(modifier = Modifier.height(12.dp))
