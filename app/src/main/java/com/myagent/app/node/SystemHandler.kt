  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "灵机 Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "灵机 Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "灵机 Active")
      }
