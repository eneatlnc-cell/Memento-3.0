...
class SecurePrefs(
  ...
  // MyAgent website JWT token
  private val _myagentToken = MutableStateFlow(plainPrefs.getString("myagent.jwt_token", null))
  val myagentToken: StateFlow<String?> = _myagentToken

  fun setMyagentToken(token: String?) {
    if (token != null) {
      plainPrefs.edit { putString("myagent.jwt_token", token) }
    } else {
      plainPrefs.edit { remove("myagent.jwt_token") }
    }
    _myagentToken.value = token
  }
